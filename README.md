...
A multi-page portfolio built with **HTML5 (XHTML syntax), CSS3 and JSON only** 
1. Overview
...
skills.html	ItemList of skill ListItems
...
Non-technical editors. If a department administrator, a student society or a client needs to publish without touching code, a CMS's WYSIWYG editor and role-based permissions are the entire value proposition.
Update frequency above roughly weekly. Once content changes faster than a developer can reasonably review commits, the overhead of the manual workflow exceeds the overhead of maintaining a CMS.
Team size greater than two or three contributors. Concurrent editing, drafts, approval workflows and audit trails are solved problems in a CMS and awkward to reconstruct in Git for non-developers.
Dynamic features. Search, comments, form submissions stored server-side, personalisation or an editorial calendar require server logic that a static site cannot provide.
...
6. Deployment
The repository is hosted on GitHub and published with GitHub Pages from the main branch root. A .nojekyll file disables Jekyll processing so files are served exactly as committed. HTTPS is enforced in the repository's Pages settings.
